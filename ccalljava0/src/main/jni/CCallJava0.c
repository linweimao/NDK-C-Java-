 ...
 */
JNIEXPORT void JNICALL Java_com_lwm_ccalljava0_JNITest_callbackAdd
 ...
 * 没有参数 也没有返回值
 */
JNIEXPORT void JNICALL Java_com_lwm_ccalljava0_JNITest_callbackHelloFromJava
        (JNIEnv* env, jobject jobj){
 ...
 * 有 String 参数 但是没有返回值
 */
JNIEXPORT void JNICALL Java_com_lwm_ccalljava0_JNITest_callbackPrintString
        (JNIEnv* env, jobject jobj){
 ...
 * 静态方法不需要实例化
 */
JNIEXPORT void JNICALL Java_com_lwm_ccalljava0_JNITest_callbackSayHello
        (JNIEnv* env, jobject jobj){
 ...
 */
void Java_com_lwm_ccalljava0_JNITest_callBackShowToast(JNIEnv *env, jobject instance) {

    //让 C 代码调用 MainActivity 里面的 public void showToast()方法，那么就要得到 MainActivity 的实例
    //1、得到字节码(字节码是用于实例化这个类)
    jclass jclazz = (*env)->FindClass(env, "com/lwm/ccalljava0/MainActivity");
